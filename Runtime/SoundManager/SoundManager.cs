...
namespace BB.Framework
{
    ...
    public class SoundManager : MonoBehaviour
    {
        ...
        [Header("Catalog + Mixer")] [SerializeField]
        ...
        [Header("Pooling")] [SerializeField]
        private AudioSource audioSourcePrefab; // lightweight prefab (no clip), output set to SFX group by default
        ...
        [Header("Music")] [SerializeField] private AudioSource musicA;
        ...
        void StartClip(AudioSource src, Sound s)
        {
            ...
            src.Play();
            if (!s.loop)
                StartCoroutine(ReturnWhenDone(src, s.id, clip.length / Mathf.Max(0.01f, Mathf.Abs(src.pitch))));
        ...
        IEnumerator ReturnWhenDone(AudioSource src, string id, float duration)
        {
            ...
            float t = 0f;
            while (t < duration && src.isPlaying)
            {
        ...
        {
            if (!TryGetSound(id, out var s) || !s.isMusic)
            {
                Debug.LogWarning($"Music id '{id}' missing or not marked isMusic.");
                return;
            }
